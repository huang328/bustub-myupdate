...
<<<<<<< HEAD
          ...
          break;
        case AggregationType::CountAggregate:
          if (!input.values[i].is_null()) {
            result->values[i] = ValueFactory::GetIntegerValue(result->values[i].GetAs<int64_t>() + 1);
          }
          break;
        case AggregationType::SumAggregate:
          if (!input.values[i].is_null()) {
            result->values[i] = ValueFactory::GetIntegerValue(result->values[i].GetAs<int64_t>() + input.values[i].GetAs<int64_t>());
          }
          break;
        case AggregationType::MinAggregate:
