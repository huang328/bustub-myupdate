...
auto ExtendibleHashTable<K, V>::IndexOf(const K &key) -> size_t {
  // 将mask最后global_depth_位 置1
  int mask = (1 << global_depth_) - 1;
  // mask与哈希函数按位与只取哈希函数的最后global_depth_位
...
template class ExtendibleHashTable<page_id_t, Page *>; // 页id对应的页表
template class ExtendibleHashTable<Page *, std::list<Page *>::iterator>; // 页与对应装k/v的list
template class ExtendibleHashTable<int, int>; // K/V键值对
// test purpose
